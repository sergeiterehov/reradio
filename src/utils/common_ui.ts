...
export const common_ui = {
  ...
  vox_delay: (ref: _GetSetNumber, config: { from: number; to: number; step: number }): UI.Field.Slider => ({
    type: "slider",
    id: "vox_delay",
    name: t("vox_delay"),
    description: t("vox_delay_tooltip"),
    tab: UITab.VOX,
    min: 0,
    max: (config.to - config.from) / config.step,
    label: (val) => t("seconds_value", { replace: { value: (config.from + val * config.step).toFixed(1) } }),
    get: () => ref.get(),
    set: (val) => ref.set(Number(val)),
  }),
  ...
  pow_tot: (ref: _GetSetNumber, config: { from: number; to: number; step: number }): UI.Field.Slider => ({
    ...
    min: 0,
    max: (config.to - config.from) / config.step,
    label: (val) => (val ? t("seconds_value", { replace: { value: config.from + val * config.step } }) : t("off")),
    get: () => ref.get(),
    set: (val) => ref.set(Number(val)),
  }),
  ...
  timeout_alarm: (ref: _GetSetNumber, config: { from: number; to: number; step: number }): UI.Field.Slider => ({
    type: "slider",
    id: "timeout_alarm",
    name: t("timeout_alarm"),
    description: t("timeout_alarm_tooltip"),
    tab: UITab.Exchange,
    min: 0,
    max: (config.to - config.from) / config.step,
    label: (val) => (val ? t("seconds_value", { replace: { value: config.from + val * config.step } }) : t("off")),
